				...
				<c:forEach var="reservationListDto" items="${rezList}">
					...
					<div class="row res-line p-4" style="margin-left: 100px;">
						...
						<div class="col-2 text-end">
							...
							<c:choose>
								<c:when test="${reservationListDto.reviewWritten}">
									<!-- 리뷰가 이미 작성된 경우: 버튼을 숨김 -->
									<div class="row">리뷰가 이미 작성되었습니다.</div>
								</c:when>
								<c:otherwise>
									<!-- 리뷰가 작성되지 않은 경우: 버튼을 표시 -->
									<div class="row">
										<a class="btn btn-warning btn-sm open-modal-review"
											href="/customer/reviewWrite?resNo=${reservationListDto.rezResNo}">리뷰작성</a>
									</div>
								</c:otherwise>
							</c:choose>
						</div>
