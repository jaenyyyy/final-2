...
window.contextPath = "${pageContext.request.contextPath}";
	function goToDetail(resNo) {
	    window.location = window.contextPath+'detail?resNo=' + resNo;
	}
</script>
				...
				<div class="col-md-4">
					...
					<div class="input-group mb-3">
						...
						<input type="text" name="resName"
							value="${restaurantAdminListDto.resName}" class="form-control"
							autocomplete="off">
					</div>
		...
		<div class="row" style="margin-left: 10%; margin-right: 10%">
			<div class="col-md-6 mt-4">
				<h1 class="text-md-start text-center">
					<i class="fa-solid fa-circle-check" style="color: #ffb416;"></i>매장
					관리
				</h1>
			</div>
		</div>
				...
				<table class="table table-hover justify-content-center text-center">
					...
					<tbody>
						<c:forEach var="restaurantAdminListDto" items="${list}">
							<tr onClick="goToDetail(${restaurantAdminListDto.resNo})"
			...
			<ul class="pagination justify-content-center">
				...
				<c:if test="${!vo.first}">
					<li class="page-item"><a class="page-link"
						href="list?${vo.prevQueryString}" aria-label="Previous"> <span
				...
				<c:forEach var="i" begin="${vo.begin}" end="${vo.end}" step="1">
					<li class="page-item ${vo.page == i ? 'active' : ''}"><c:choose>
							...
							<c:otherwise>
								<a class="page-link" href="list?${vo.getQueryString(i)}"
				...
				<c:if test="${!vo.last}">
					<li class="page-item"><a class="page-link"
						href="list?${vo.nextQueryString}" aria-label="Next"> <span
